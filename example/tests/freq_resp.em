...
for cfg in data:
    ...
    h2 = model.getWave(array([1.0 / T] + [0.0] * (len(h)/4 - 1)), row_size=len(h)/4)
    for i in range(3):
        h2 = concatenate((h2, model.getWave(array([0.0] * (len(h)/4)))))
    ...
    semilogx(f / 1.e9, 20. * log10(abs(H[:len(H)/2])),        label=cfg_name+'_Init',    color=color_main)
    semilogx(f / 1.e9, 20. * log10(abs(H2[:len(H2)/2])), '.', label=cfg_name+'_GetWave', color=color_main)
    if(reference):
        try:
            if(ref is None):
