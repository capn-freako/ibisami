...
@{
from pylab import *
from numpy import cumsum, concatenate, pad
...
for cfg in data:
    ...
    items = list(params[1].items())
    items.sort(reverse=True)  # Note: This step is MANDATORY!
    for item in items:
        exec ('initializer.' + item[0] + ' = ' + repr(item[1]))
    model.initialize(initializer)
    print(f'        <block name="Model Initialization ({cfg_name})" type="text">')
    print("MUT:")
    print(model.msg)
    print(model.ami_params_out)
    h = pad(model.initOut, (10,0))
    T = model.sample_interval
    t = array([i * T for i in range(-10, len(h)-10)])
    ...
    s2_pre  = pad(s2[half_len:], (10, 0))
    s2_plot = pad(s2_pre, (0, half_len-10), 'edge')
    h2 = diff(s2)
    H = fft(h)
    ...
    color_main = "#%02X%02X%02X" % (int(rgb_main[0] * 0xFF), int(rgb_main[1] * 0xFF), int(rgb_main[2] * 0xFF))
    color_ref  = "#%02X%02X%02X" % (int(rgb_ref[0]  * 0xFF), int(rgb_ref[1]  * 0xFF), int(rgb_ref[2]  * 0xFF))
    figure(1)
